...
<div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
    ...
    <div class="flex items-center justify-between mb-6">
        <div>
            ...
            <p class="text-sm text-gray-500 mt-1">Semua notifikasi Anda</p>
        </div>
    ...
    <div class="bg-white rounded-xl border border-gray-200 overflow-hidden">
            ...
            <div class="px-6 py-4 border-b border-gray-100 last:border-0 hover:bg-gray-50 transition-colors {{ $notification->read_at ? 'opacity-60' : '' }}">
                <div class="flex items-start gap-4">
                    <div class="flex-shrink-0 mt-0.5">
                        <div class="w-10 h-10 rounded-full {{ $notification->read_at ? 'bg-gray-100' : 'bg-primary-100' }} flex items-center justify-center">
                            <i class="fas fa-bell {{ $notification->read_at ? 'text-gray-400' : 'text-primary-600' }} text-sm"></i>
                        </div>
                    </div>
                    ...
                    @if(!$notification->read_at)
                        <div class="flex-shrink-0">
                            <span class="w-2.5 h-2.5 bg-primary-500 rounded-full block"></span>
                        </div>
                    @endif
                </div>
            </div>
        @empty
            <div class="px-6 py-16 text-center">
                ...
                <p class="text-sm text-gray-500">Notifikasi akan muncul di sini ketika ada aktivitas terkait lelang Anda.</p>
            </div>
        @endforelse
    </div>
