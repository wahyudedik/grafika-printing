        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="text" name="name" value="{{ old('name', $adminFee->name) }}" placeholder="Contoh: Biaya Admin Lelang"
            ...
            <div class="mt-6">
                ...
                <textarea name="description" rows="3" placeholder="Deskripsi pengaturan biaya admin"
                    class="block w-full rounded-lg border {{ $errors->has('description') ? 'border-red-300 focus:border-red-500 focus:ring-red-500' : 'border-gray-300 focus:border-primary-500 focus:ring-primary-500' }} px-3 py-2 text-sm shadow-sm focus:ring-2 focus:ring-opacity-50">{{ old('description', $adminFee->description) }}</textarea>
                @error('description')
                    <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mt-6">
                <div>
                    ...
                    <select name="type" id="type"
                        ...
                        <option value="fixed" {{ old('type', $adminFee->type) == 'fixed' ? 'selected' : '' }}>Tetap (Rp)</option>
                        <option value="percentage" {{ old('type', $adminFee->type) == 'percentage' ? 'selected' : '' }}>Persentase (%)</option>
                    </select>
                    @error('type')
                        <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                    @enderror
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mt-6">
                <div>
                    ...
                    <input type="number" name="minimum_amount" value="{{ old('minimum_amount', $adminFee->minimum_amount) }}" step="0.01" min="0" placeholder="0"
                ...
                <div>
                    ...
                    <input type="number" name="maximum_amount" value="{{ old('maximum_amount', $adminFee->maximum_amount) }}" step="0.01" min="0" placeholder="Tidak terbatas"
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mt-6">
                <div>
                    ...
                    <input type="datetime-local" name="effective_from"
                        value="{{ old('effective_from', $adminFee->effective_from ? $adminFee->effective_from->format('Y-m-d\TH:i') : '') }}"
                ...
                <div>
                    ...
                    <input type="datetime-local" name="effective_until"
                        value="{{ old('effective_until', $adminFee->effective_until ? $adminFee->effective_until->format('Y-m-d\TH:i') : '') }}"
            ...
            <div class="mt-6">
                <label class="flex items-center gap-2 cursor-pointer">
                    <input type="checkbox" name="is_active" value="1" {{ old('is_active', $adminFee->is_active) ? 'checked' : '' }}
