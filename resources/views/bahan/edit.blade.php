    ...
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="grid grid-cols-1 gap-6">
            <div>
                ...
                <form action="{{ route('vendor.materials.update', $bahan->id) }}" method="POST"
                    onsubmit="showLoading('Memperbarui bahan...')">
                    @csrf
                    @method('PUT')
                    <div class="bg-white rounded-xl shadow-sm">
                        <div class="px-6 py-4 border-b border-gray-200">
                            <h3 class="text-lg font-semibold text-gray-900">Edit Bahan</h3>
                        </div>
                        <div class="px-6 py-4">
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                ...
                                <div>
                                    ...
                                    <div class="flex">
                                        ...
                                        <input type="number" step="0.01" name="hpp"
                                            value="{{ old('hpp', $bahan->hpp) }}"
                            ...
                            <div class="mt-6">
                                <h4 class="text-sm font-semibold text-gray-900">Harga Grosir</h4>
                                <p class="text-sm text-gray-500 mb-3">Atur tier harga grosir untuk pembelian dalam jumlah banyak</p>
                                ...
                                <div id="wholesale-container">
                                    ...
                                    @if ($bahan->wholesalePrices && $bahan->wholesalePrices->count() > 0)
                                        @foreach ($bahan->wholesalePrices as $price)
                                            <div class="grid grid-cols-1 md:grid-cols-12 gap-3 mb-3 wholesale-row items-end"
                                                ...
                                                <div class="md:col-span-3">
                                                    ...
                                                    <input type="number" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary focus:border-primary text-sm"
                                                        name="wholesale_min_qty[]" value="{{ $price->min_quantity }}"
                                                        placeholder="Min quantity" min="1" required>
                                                </div>
                                                <div class="md:col-span-3">
                                                    <label class="block text-sm font-medium text-gray-700 mb-1">Maximum Qty</label>
                                                    <input type="number" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary focus:border-primary text-sm"
                                                        name="wholesale_max_qty[]" value="{{ $price->max_quantity }}"
                                                        placeholder="Max quantity or leave empty">
                                                    <p class="mt-1 text-xs text-gray-500">Kosongkan untuk unlimited</p>
                                                </div>
                                                <div class="md:col-span-4">
                                                    <label class="block text-sm font-medium text-gray-700 mb-1">Harga (Rp)</label>
                                                    <input type="number" step="0.01" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary focus:border-primary text-sm"
                                                        name="wholesale_price[]" value="{{ $price->harga }}"
                                                        placeholder="Wholesale price" required>
                                                </div>
                                                <div class="md:col-span-2 flex items-end">
                                                    <button type="button"
                                                        class="inline-flex items-center gap-1 border border-red-500 text-red-500 px-3 py-2 rounded-lg text-sm font-medium hover:bg-red-50 transition-colors remove-existing-row"
            ...
            document.addEventListener('DOMContentLoaded', function() {
                ...
                function addWholesalePriceRow(minQty = '', maxQty = '', price = '') {
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-12 gap-3 mb-3 wholesale-row items-end" id="${rowId}">
                        <div class="md:col-span-3">
                            <label class="block text-sm font-medium text-gray-700 mb-1">Minimum Qty</label>
                            <input type="number" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary focus:border-primary text-sm" name="new_wholesale_min_qty[]" value="${minQty}" placeholder="Min quantity" min="1" required>
                        </div>
                        <div class="md:col-span-3">
                            <label class="block text-sm font-medium text-gray-700 mb-1">Maximum Qty</label>
                            <input type="number" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary focus:border-primary text-sm" name="new_wholesale_max_qty[]" value="${maxQty}" placeholder="Max quantity or leave empty">
                            <p class="mt-1 text-xs text-gray-500">Kosongkan untuk unlimited</p>
                        </div>
                        <div class="md:col-span-4">
                            <label class="block text-sm font-medium text-gray-700 mb-1">Harga (Rp)</label>
                            <input type="number" step="0.01" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary focus:border-primary text-sm" name="new_wholesale_price[]" value="${price}" placeholder="Wholesale price" required>
                        </div>
                        <div class="md:col-span-2 flex items-end">
